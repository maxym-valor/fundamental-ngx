...
import { currentDay, currentYear, invalidDate, getCurrentItemIndex, getCurrentMonth } from '../fixtures/testData/date-picker-tags'
...
describe('Datetime picker suite', function () {
    ...
    const {
        defaultExample, formExample, rangeExample, disabledExample, positionExample, allowNullExample, formRangeExample, formattingExample, disableFuncExample,
    ...
    function checkChoosingDate(section: string): void {
        ...
        click(section + calendarIcon);
        clickNextElement(currentItem);
        section === formattingExample ? chosenDate = `${getCurrentMonth(true)}/${currentDay + 1}/${currentYear.toString().slice(2)}` : chosenDate = `${getCurrentMonth(false)}/${currentDay + 1}/${currentYear}`;

        expect(getValue(section + calendarInput)).toContain(chosenDate, `input does not contain chosen value`);
        click(section + calendarIcon);
    }
